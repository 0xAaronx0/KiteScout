...
export async function GET(req: NextRequest) {
  ...
  // Build a fallback chain: full string → country portion → first token
  const parts = q.split(',').map(s => s.trim()).filter(Boolean);
  const candidates = [q];
  if (parts.length > 1) candidates.push(parts[parts.length - 1]); // country
  if (parts.length > 2) candidates.push(parts[0]);                 // spot only

  for (const candidate of candidates) {
    try {
      const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(candidate)}&format=json&limit=1`;
