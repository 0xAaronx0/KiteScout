        ...
        <header className="bg-white border-b border-slate-200 sticky top-0 z-20">
          <div className="px-4 py-2.5 flex items-center gap-3">
            <button onClick={reset}
              className="text-sky-600 text-sm font-medium hover:text-sky-700 flex items-center gap-1 shrink-0">
              ← Back
            </button>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/logo.svg" alt="KiteScout Cruises" className="w-7 h-7 shrink-0" />
            <h1 className="font-bold text-slate-900 truncate flex-1 min-w-0">
              Kite Cruises — {searchedDestination}
            </h1>
            {providers.length > 0 && (
              <span className="text-xs text-slate-400 font-medium shrink-0 tabular-nums">
                {Math.min(swipeIndex + 1, providers.length)} / {providers.length}
              </span>
            )}
          </div>
          {/* Thin progress track replaces the old separate counter row */}
          {providers.length > 0 && (
            <div className="h-0.5 bg-slate-100">
              <div className="h-full bg-sky-500 transition-all duration-300"
                style={{ width: `${(swipeIndex / providers.length) * 100}%` }} />
            </div>
          )}
        </header>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <footer className="bg-slate-900 text-slate-500 text-xs text-center py-3">
        🪁 KiteScout Cruises — AI-powered kite cruise finder
      </footer>
    </div>
  );
}
