...
/**
 * Geocode an itinerary spot, falling back to progressively simpler queries.
 * Exported so backfill scripts can reuse the exact pipeline behavior.
 */
...
async function processProvider(cp: {
  ...
  for (const offer of offers) {
    ...
    for (let i = 0; i < rawSpots.length; i++) {
      ...
      const coords = await geocodeSpot(s.name.trim(), region, country);
      spots.push({
        name: s.name.trim(),
        country,
        region,
    ...
    if (spots.length > 0) {
      const hit = spots.filter(sp => sp.lat != null).length;
      const missing = spots.filter(sp => sp.lat == null).map(sp => sp.name);
      console.log(`   spots   : geocoded ${hit}/${spots.length}${missing.length ? `  (no coords: ${missing.join(', ')})` : ''}`);
    }
